...
async def executor(client: AddyBot, message: Message):
...
@AddyBot.on_cb("runtime")
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@AddyBot.on_cb("forceclose")
...
async def shellrunner(client: AddyBot, message: Message):
    if len(message.command) < 2:
